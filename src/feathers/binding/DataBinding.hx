...
class DataBinding {
	...
	macro public static function bind(source:Expr, destination:Expr, document:Expr = null):Expr {
				...
				case TInst(t, params):
					...
					while (field == null && currentType != null) {
						field = Lambda.find(currentType.fields.get(), item -> {
							return item.name == sourceFieldName;
						});
		...
		if (hasDocument) {
			initCode = macro {
				function document_addedToStageHandler(event:Event):Void {
					activateBinding();
				}
				function document_removedFromStageHandler(event:Event):Void {
					deactivateBinding();
				}
				$document.addEventListener(Event.ADDED_TO_STAGE, document_addedToStageHandler, false, 0, true);
				$document.addEventListener(Event.REMOVED_FROM_STAGE, document_removedFromStageHandler, false, 0, true);
		...
		return macro {
			(function():Void {
				...
				function bindingHandler(event:Event):Void {
					executeBinding();
				}
